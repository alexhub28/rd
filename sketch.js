...
function drawChart() {
  ...
  d3.csv("RD_2025.csv").then(raw => {
    ...
    // ⭐ text-anchor "start" (pas "middle") + ancrage en bas de l'axe :
    // les 3 lignes commencent toutes au même point (en bas, près de
    // l'axe) et remontent, au lieu d'être centrées indépendamment sur
    // toute la hauteur — ce qui les faisait déborder/se chevaucher,
    // en particulier la ligne allemande (la plus longue).
    function axisTitle(x, textBlock) {
      ...
      const titleText = g.append("text")
        .attr("transform", `translate(${x}, ${innerHeight}) rotate(-90)`)
