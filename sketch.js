...
function drawChart() {
  ...
  d3.csv("RD_2025.csv").then(raw => {
    ...
    // ⭐ Espace réservé en haut pour les légendes d'axes (voir plus bas) :
    // 3 lignes courtes, non pivotées, plutôt qu'un texte à -90° qui
    // restait illisible pour des libellés aussi longs.
    const captionHeight = isMobile ? 38 : 42;
    const margin = {
      top: captionHeight + 10,
    ...
    // --- Légendes d'axes, non pivotées ---
    // ⭐ Un texte pivoté à -90° restait illisible malgré plusieurs
    // ajustements (chevauchement / dépassement de hauteur), la ligne
    // allemande en particulier. 3 courtes lignes horizontales au-dessus
    // du graphique, alignées à gauche / à droite sur chaque axe, sont
    // beaucoup plus fiables à lire.
    const captionLineGap = isMobile ? 11 : 12.5;

    function axisCaption(textBlock, anchorX, textAnchor) {
      const lines = textBlock.split("\n");
      lines.forEach((line, i) => {
        g.append("text")
          .attr("x", anchorX)
          .attr("y", -captionHeight + 8 + i * captionLineGap)
    ...
    axisCaption(AXIS_LEFT_LABEL, 0, "start");
    axisCaption(AXIS_RIGHT_LABEL, innerWidth, "end");

    // --- Ligne des jours de service (axe droit), tracé progressif ---
    // ⭐ Dessinée AVANT les barres/valeurs (ci-dessous), pour qu'elle ne
    // passe jamais par-dessus les chiffres — l'ordre de dessin SVG place
    // toujours les éléments ajoutés en dernier au premier plan.
    ...
    SERIES.forEach((s, si) => {
      ...
      // ⭐ Infobulle native (survol de la barre) : la valeur interne
      // reste disponible au survol, sans encombrer l'affichage par défaut.
      bars.append("title")
        .text(d => `${s.label}\n${formatSwiss(d[s.key])} (dont ${formatSwiss(d[s.internKey])} au total)`);
      ...
      // ⭐ Uniquement la valeur principale (la valeur interne entre
      // parenthèses a été retirée de l'affichage par défaut, disponible
      // au survol via l'infobulle ci-dessus) : avec 5 barres fines par
      // groupe, l'ancien "563 (613)" débordait sur les barres/valeurs
      // voisines. Un contour blanc garantit aussi la lisibilité même
      // sur la barre "Total", très foncée.
