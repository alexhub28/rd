...
function drawChart() {
  ...
  d3.csv("RD_2025.csv").then(raw => {
    ...
    // Une fois le texte pivoté à -90°, il se lit de bas en haut : les 3
    // lignes doivent donc être décalées HORIZONTALEMENT (perpendiculaire
    // au sens de lecture), pas verticalement — sinon elles se superposent
    // toutes au même endroit, comme dans la version précédente.
    function axisTitle(x, textBlock) {
      const lines = textBlock.split("\n");
      const lineGap = isMobile ? 10.5 : 12.5;
      const startOffset = -((lines.length - 1) / 2) * lineGap;

      lines.forEach((line, i) => {
        g.append("text")
          .attr("transform", `translate(${x + startOffset + i * lineGap}, ${innerHeight / 2}) rotate(-90)`)
    ...
    // ⭐ Embed compact : une seule ligne trilingue par item (au lieu de 3
    // lignes empilées), pour garder la légende basse et tenir dans 500px.
    function appendLegendText(container, label) {
      container.append("div").attr("class", "legend-text").text(label);
    }
